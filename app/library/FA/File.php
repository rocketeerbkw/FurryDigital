...
class File
{
    ...
    public static function getFilePath($file_name)
    {
        ...
        if (is_readable($file_name) || stristr($file_name, FA_UPLOAD_FOLDER) !== FALSE)
            return $file_name;
        else
            return FA_UPLOAD_FOLDER.DIRECTORY_SEPARATOR.str_replace('..','',$file_name);
    }
    ...
    public static function moveUploadedFile($uploaded_file, $file_name = '')
    {
        if (!self::isValidFile($uploaded_file))
        {
            switch($uploaded_file['error'])
            {
                case UPLOAD_ERR_INI_SIZE:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: The file you are attempting to upload is larger than allowed (upload_max_filesize).');
                    break;
                
                case UPLOAD_ERR_FORM_SIZE:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: The file you are attempting to upload is larger than allowed (MAX_FILE_SIZE).');
                    break;
                    
                case UPLOAD_ERR_PARTIAL:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: The file you are attempting to upload was only partially uploaded.');
                    break;
                
                case UPLOAD_ERR_NO_FILE:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: No file was uploaded.');
                    break;
                
                case UPLOAD_ERR_NO_TMP_DIR:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: Missing a temporary folder.');
                    break;
                
                case UPLOAD_ERR_CANT_WRITE:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: Failed to write file to disk.');
                    break;
                
                case UPLOAD_ERR_EXTENSION:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: Upload stopped by extension.');
                    break;
                    
                default:
                    throw new \FA\Exception\DisplayOnly('File Upload Error: No file was specified.');
                    break;
            }
            exit;
        }
        ...
        {
            throw new \FA\Exception\DisplayOnly('File Upload Error: Could not upload the file requested.');
            exit;
        }
    }
